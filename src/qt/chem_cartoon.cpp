...
int ChemWidget::drawCartoon(int imol, QString molnam, char chain, int mode, float ribbon_width, float coil_radius) {
    ...
    chainQuery cq = chainQuery();
    cq.iter(imol, chain, FILTER_CARTOON);
    int numres = cq.count() / 2; // CA and O make two rows per residue
    if (numres < 1)  {
    ...
    float (*CA)[3]          = new float[numres+2][3];
    float (*O)[3]           = new float[numres+2][3];
    float (*guide_zero)[3]  = new float[numres+2][3];
    float (*guide_plus)[3]  = new float[numres+2][3];
    float (*guide_minus)[3] = new float[numres+2][3];
    float (*astrand)[3], (*bstrand)[3], (*fliptmp)[3];  // pointers to faciliate swapping sides for strands
    std::string sstype = std::string(numres+2,'?');//chainQuery::getChainSS(imol, chain, numres);
    
    // get the CA and O coords
    int ires = 0;
    int mres = NORESNUM; // previous residue number
    int njump = 0; // possible missing residue's coords
    for ( /* cq already init'ed */ ; cq.next(); ) {
    ...
    // back-extrapolate t0 #0 and to mres+1 
    // to smoothly begin and end the cartoon
    // extrapolate missing coords
    float xinc = (CA[1][0] - CA[2][0]);
    float yinc = (CA[1][1] - CA[2][1]);
    float zinc = (CA[1][2] - CA[2][2]);
    CA[0][0] = CA[1][0] + xinc;
    CA[0][1] = CA[1][1] + yinc;
    CA[0][2] = CA[1][2] + zinc;
    O[0][0] = O[1][0] + xinc;
    ...
    O[0][2] = O[1][2] + zinc;
    sstype[0] = sstype[1];
    xinc = (CA[ires][0] - CA[ires-1][0]);
    yinc = (CA[ires][1] - CA[ires-1][1]);
    zinc = (CA[ires][2] - CA[ires-1][2]);
    CA[ires+1][0] = CA[ires][0] + xinc;
    CA[ires+1][1] = CA[ires][1] + yinc;
    CA[ires+1][2] = CA[ires][2] + zinc;
    O[ires+1][0] = O[ires][0] + xinc;
...
*/
    ...
    // now draw the ribbon
    makeRibbon(nguide, (float *)guide_minus, (float *)guide_plus, (float *)guide_zero, sstype, numres, coil_radius, ribbon_width, false, true);
...
int ChemWidget::makeRibbon(int nguide, float *guide_minus, float *guide_plus, float *guide_zero, std::string sstype, int hires, float coil_radius, float width, bool endcaps, bool showUnknown) {
    //  makes coils and ribbons, depending on the secondary struture codes 'H', 'S', 'T', '?'/unknown, 'X'/extrapolated
    int err = 0;
    ...
    while (ires <= hires) {
        ...
        if (n == -1)  {
            // all remaining types the same
            nspan = hires - ires + 1;
            len = spline_len - begin;
        } else {
            nspan = n - ires;
            if (sstype[ires] == '?' || sstype[ires] == 'T') {
        ...
        } else if (sstype[ires] == '?' && showUnknown) {
